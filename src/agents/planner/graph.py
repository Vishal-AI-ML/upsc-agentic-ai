...
from src.agents.planner.constants import (
    get_attempt_guidance,
    get_optional_context,
    get_weak_context,
    get_weak_plan_instruction,
)
...
def _fetch_upsc_prelims_date(attempt_year: int) -> str:
    ...
    try:
        ...
        if tool:
            results = tool.invoke(
                f"UPSC CSE Prelims {attempt_year} exam date official notification"
            )
            for r in results or []:
...
def generate_plan(
    ...
    live_upsc_date = (
        ...
        if live_date
        else "Could not fetch live date — verify at upsc.gov.in"
    )
    ...
    if live_date:
        try:
            ...
            months_left = max(
                0, (prelims_dt.year - today.year) * 12 + (prelims_dt.month - today.month)
            )
        except Exception:
    ...
    if months_left == 0:
        ...
        timeline_msg = (
            "Under 6 months. Intensive — cover all high-priority topics with daily testing."
        )
    elif months_left <= 12:
